 ...
 */
Machine::Machine(QObject *parent, const QString n, QString v, QString a,
                 QString d, int t, int nb, QString onamesm, int ni,
                 QString inames, int no, QString onames, QFont sf, QFont tf,
                 int atype)
    : QObject(parent) {
  name = n;
  version = v;
...
bool Machine::addState(const QString sname, QString sdescription, int code,
                       ...
                       double scale, QPen pen, bool endstate,
                       bool withundo /*=true*/, QString entry_actions /*=""*/,
                       QString exit_actions /*=""*/) {
  GState *s = new GState(this, sname, sdescription, code, mooreout, xpos, ypos,
                         radius, pen, endstate, entry_actions, exit_actions);
  state_list.append(s);
...
GObject *Machine::getObject(QPoint p, double scale, int &type) {
  ...
  if (initial_transition &&
      initial_transition->contains(p.x() / scale, p.y() / scale)) {
    type = ITransitionT;
...
void Machine::updateCanvasSize(int x, int y, double scale) {
  if (x > canvas_size.width()) {
    canvas_size.setWidth(x);
    emit newCanvasSize(int(canvas_size.width() * scale),
                       int(canvas_size.height() * scale));
  }
  ...
  for (i = names.constBegin(); i != names.constEnd(); ++i) {
    ...
    if (pos1 != -1 && pos2 != -1) {
      if (indexStarted && arrayName != oldArrayName) {
        ret += (arrayName + "[" + QString::number(indexstart) + ".." +
                QString::number(indexend) + "]");
    ...
    if ((i + 1 == names.constEnd() || ok == false) && indexStarted == true) {
      if (indexend == -1)
        ...
        ret += oldArrayName + "[" + QString::number(indexstart) + ".." +
               QString::number(indexend) + "]";
