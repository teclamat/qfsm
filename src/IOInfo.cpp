...
bool IOInfo::matches(IOInfo *io) {
  ...
  for (i2 = l2.constBegin(); i2 != l2.constEnd(); ++i2) {
    ...
    for (i1 = l1.constBegin(); i1 != l1.constEnd(); ++i1) {
      if (((IOInfoBin *)(*i1))->getInfo() == ((IOInfoBin *)(*i2))->getInfo()) {
        // qDebug("match: %s (in %s) matches %s",
        // (*i1)->convertToString().latin1(), convertToString().latin1(),
        // (*i2)->convertToString().latin1());
        io_matches = true;
        break;
      }
    }
...
void IOInfo::expandList(IOInfoList &input, IOInfoList &list,
                        bool resolve_invert) {
  ...
  */
  for (; evit.hasNext();) {
    ...
    } else // not inverted
    {
      ...
      {
        open_counter--;
        if (!io->equals(previo) || (io->equals(previo) && previo != NULL &&
                                    io->getRole() != previo->getRole()))
          end = io;
