...
/// Constructor
IOInfoASCII::IOInfoASCII(IOType t, unsigned char i) : IOInfo(t) {
...
/// Constructor
IOInfoASCII::IOInfoASCII(IOType t, QString i) : IOInfo(t) { info = i; }
...
QString IOInfoASCII::convertToString(Machine * /*m=NULL*/,
                                     Options * /*opt=NULL*/) const {
  return info;
}

QString IOInfoASCII::convertToComparableString(Machine * /*m=NULL*/,
                                               Options * /*opt=NULL*/) const {
  /*
  IOInfoList list;
...
QString IOInfoASCII::convertToBinStr(Machine * /*m=NULL*/,
                                     Options * /*opt=NULL*/) const {
  unsigned char ascii[MAX_CHARARRAY_LENGTH];
  int alen;
...
//  char lc, rc;
  ...
  while (i<len)
  {
    ...
    if (ctmp=='\\')		// escape sequence
    {
      ...
      {
        if (cnext=='0')		// hex code (\0xx where xx is the hex code of
the character)
        {
          if (i>=len-3)
          {
    ...
    else if (ctmp=='-')		// range
    {
      ...
      if (cnext=='\\')		// last char. of range is escape character
      {
        ...
        {
          if (cnext=='0')	// hex code (\0xx where xx is the hex code of the
character)
          {
            if (i>=len-4)
            {
 ...
 */
int IOInfoASCII::findChar(const unsigned char *ascii, int len,
                          unsigned char c) {
  for (int i = 0; i < len; i++) {
    if (ascii[i] == c)
      return i;
...
IOInfo *IOInfoASCII::clone() const { return new IOInfoASCII(*this); }
...
void IOInfoASCII::split(IOInfoList &list, GState *) const { split(list, true); }
...
bool IOInfoASCII::expandList(IOInfoList& input, IOInfoList& list)
{
  ...
  for(;thisit.current(); ++thisit)
  {
    ...
    if (!list.isEmpty())
    {
      ...
      {
        io = it.current();
        qDebug("\nsplitting %s and %s", thisio->convertToString().latin1(),
io->convertToString().latin1());
        ...
        for(; resit.current(); ++resit)
        {
          ...
          if (!newlist.contains(resit.current()))
          {
            ...
            qDebug("element %s appended",
resit.current()->convertToString().latin1());
          }
          else
            qDebug("element %s already in list",
resit.current()->convertToString().latin1());
        }
      }
    }
