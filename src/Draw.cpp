 ...
 */
Draw::Draw(QObject *parent, Options *opt) : QObject(parent) {
  options = opt;
...
void Draw::drawStates(Machine *m, QPainter *p, int contx, int conty,
  ...
  for (; i.hasPrevious();) {
    ...
    if (!s->isDeleted()) {
      ...
      if (s->isFinalState())
        p->drawEllipse(int(xpos - r + 4), int(ypos - r + 4), 2 * r - 8,
                       2 * r - 8);
      ...
      if (m->getType() != Text &&
        ...
        p->drawLine(int(xpos - r + dist), int(ypos), int(xpos + r - dist),
                    int(ypos));
      ...
      if (s->isSelected() && p->device()->devType() != QInternal::Printer) {
        ...
        if (bscale > 1.0)
          bscale = 1.0;
        p->setPen(selpen);
        p->drawRect((int)(int(xpos - r) - 3 / bscale),
                    (int)(int(ypos - r) - 3 / bscale), int(2 * r + 6 / bscale),
                    int(2 * r + 6 / bscale));
      }
    }
  }
  ...
  for (; is.hasNext();) {
    ...
    for (; itrans.hasNext();) {
      ...
      if (!t->isDeleted() && !t->isDragged()) {
        ...
        if (t->getStart() == ph || t->getEnd() == ph || t->getStart() == NULL ||
            t->getEnd() == NULL)
          p->setPen(redpen);
        else
          p->setPen(pen);
        ...
        if (t->isSelected() && p->device()->devType() != QInternal::Printer) {
          ...
          p->drawRect((int)(xpos - rectsize), (int)(ypos - rectsize),
                      rectsize * 2, rectsize * 2);
          p->drawRect((int)(ex - rectsize), (int)(ey - rectsize), rectsize * 2,
                      rectsize * 2);
          p->setBrush(br_control);
          p->drawRect((int)(c1x - rectsize), (int)(c1y - rectsize),
                      rectsize * 2, rectsize * 2);
          p->drawRect((int)(c2x - rectsize), (int)(c2y - rectsize),
                      rectsize * 2, rectsize * 2);
    ...
    if (!t->isDeleted()) {
      ...
      if (t->isSelected() && !t->isDragged()) {
        ...
        p->drawRect((int)(ex - rectsize), (int)(ey - rectsize), rectsize * 2,
                    rectsize * 2);
        p->setBrush(br_control);
        p->drawRect((int)(c1x - rectsize), (int)(c1y - rectsize), rectsize * 2,
                    rectsize * 2);
        p->drawRect((int)(c2x - rectsize), (int)(c2y - rectsize), rectsize * 2,
                    rectsize * 2);
...
void Draw::drawTransition(Machine *m, GTransition *t, QPainter *p, int contx,
                          int conty, double scale, bool drawxor, bool dotted,
  ...
  */
  ...
  if (t->isSelected()) {
    ...
    p->setPen(pen);
    p->setBrush(br_end);
    p->drawRect((int)(startposx - rectsize), (int)(startposy - rectsize),
                rectsize * 2, rectsize * 2);
    p->drawRect((int)(endposx - rectsize), (int)(endposy - rectsize),
                rectsize * 2, rectsize * 2);
...

{
  ...
  if (info) {
    ...
    bound =
        p->fontMetrics().boundingRect((int)mx, (int)my, 1000, 100, // cond);
    ...
    if ((options->getViewMealyIn() || options->getViewMealyOut()) &&
        (!cond.isEmpty() &&
         !(sin.isEmpty() && sout.isEmpty()))) // don't draw empty condition
    {
      ...
      p->drawRect((int)(condx - 5), (int)(condy - 5), bound.width() + 10,
                  bound.height() + 10);
      p->setPen(pen);
...
*/
  ...
  if (!drawxor) {
    ...
    p->drawText((int)textposx, (int)textposy, boundrect.width(),
                boundrect.height(), Qt::AlignLeft | Qt::AlignVCenter,
                options->getInitialDescriptor());
    // p->drawText((int)textposx, (int)textposy, options->getStartDescriptor());
    p->setBackgroundMode(bg);
    textrect.setRect((int)textposx, (int)textposy, boundrect.width(),
                     boundrect.height());
  }
...
void Draw::drawGrid(Grid *gr, QPainter *p, int contx, int conty, double scale) {
  ...
  gr->getPoint(contx / scale, conty / scale, cx,
               cy); // get the top-left grid point
  ...
  */
  for (int i = 0; i < numpointsx; i++)
    for (int j = 0; j < numpointsy; j++) {
      guscale = gu * scale;
      p->drawLine(
          int(cx + (i - halfcross_size) * guscale), int(cy + j * guscale),
          int(cx + (i + halfcross_size) * guscale), int(cy + j * guscale));
      p->drawLine(
          int(cx + i * guscale), int(cy + (j - halfcross_size) * guscale),
          int(cx + i * guscale), int(cy + (j + halfcross_size) * guscale));
 ...
 */
void Draw::calcArrow(double p1x, double p1y, double p2x, double p2y, double &xl,
                     double &yl, double &xr, double &yr, double &xm,
                     double &ym) {
  double mg;
...
void Draw::drawHeadline(Machine *m, QPainter *p) //, double scale)
{
  ...
  //    QString date2 = QString().sprintf("%d.%d.%d", date.day(), date.month(),
  ...
  // moved text
  p->drawText(dtrect.width() / 40, dtrect.width() / 40,
              name + " \tVersion " + version);
  //    p->drawText(window.width()/scale - 140, 0, date2 + "  " +
  //    time.toString()); p->drawText(window.width()/scale - 140, 0,
  //    datetimestr);
  dtrect.setRight(dtrect.right() - dtrect.width() / 40);
  dtrect.setTop(dtrect.top() + dtrect.width() / 40);
  p->drawText(dtrect, Qt::AlignRight | Qt::AlignTop | Qt::TextSingleLine,
              datetimestr);
}
...
QRect Draw::getBoundingBox(Machine *m, QPainter *p) {
  ...
  if (m->getDrawITrans()) {
    ...
    if (!options->getInitialDescriptor().isEmpty()) {
      ...
      if (dy > 0)
        ...
        textposy = sy - boundrect.height() / 2 + exty;
      brect_tmp.setRect((int)ROUND(textposx - 0.05 * boundrect.width()),
                        (int)ROUND(textposy - 0.05 * boundrect.height()),
                        int(boundrect.width() + 0.1 * boundrect.width()),
  ...
  for (; i.hasNext();) {
    ...
    for (; j.hasNext();) {
      ...
      if (info) {
        ...
        bound = p->fontMetrics().boundingRect(
            (int)ROUND(mx), (int)ROUND(my), 1000, 100, // cond);
            Qt::TextDontClip | Qt::AlignCenter, cond);
        condx = mx - (double)bound.width() / 2;
        condy = my - (double)bound.height() / 2;
        brect_tmp.setRect((int)ROUND(condx - 5), (int)ROUND(condy - 5),
                          bound.width() + 10, bound.height() + 10);
  ...
  result.setCoords(ROUND(minx + 5), ROUND(miny + 5), ROUND(maxx - 5),
                   ROUND(maxy - 5));
  return result;
}
