...
/// Copy constructor.
IOInfoBin::IOInfoBin(const IOInfoBin &io) : IOInfo(io.getType()) {
  role = io.role;
  invert = io.invert;
  range = io.range;
...
QString IOInfoBin::convertToString(Machine *m /*=NULL*/,
...
QString IOInfoBin::convertToComparableString(Machine *m /*=NULL*/,
                                             Options *opt /*=NULL*/) const {
  return convertToString(m, opt); // + QString::number(!role);
}

QString IOInfoBin::convertToBinStr(Machine *m /*=NULL*/,
                                   Options *opt /*=NULL*/) const {
  QString result("");
...
bool IOInfoBin::convertToASCII(unsigned char *ascii, int maxlen, int &length,
...
void IOInfoBin::setASCII(const unsigned char *ascii, int asciilen,
                         int numbits) {
  Convert conv;
...
/// Sets the I/O info to the character string @a io (binary format, see
/// IOInfoBin::info)
...
IOInfo *IOInfoBin::clone() const { return new IOInfoBin(*this); }
  ...
  if (def) {
    ...
    for (int i = 0; i < len; i++) {
      ...
      while (it.hasNext()) {
        ...
        t = it.next();
        if (t->getInfo()->getInputInfo()->matches(iotmp) &&
    ...
    //    qDebug("Split default transition "+s->getStateName()+" length:
    //    "+QString::number(list.length())+" of "+QString::number(len));
  } else
    split(list, true);
}
...
void IOInfoBin::convertToBinList(IOInfoList &list,
 ...
 * io). IOInfo::matches() also works but is less efficient (especially for lots
 * of "don't care" bits).
 */
bool IOInfoBin::matches(IOInfo *io) {
  bool io_matches;
  ...
  for (i2 = l2.constBegin(); i2 != l2.constEnd(); ++i2) {
    ...
    if (isInverted()) // for inverted transitions, every inverted component
                      // needs to match (as !(A | B) is equivalent with !A & !B)
      io_matches = true;
    else
      io_matches = false;
