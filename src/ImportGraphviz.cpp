...
ImportGraphviz::ImportGraphviz(Options *opt) : Import(opt) {}
...
qfsm::Project* ImportGraphviz::doImport()
{
  ...
  bb.setCoords((int)bbl.at(0).toFloat(), (int)bbl.at(1).toFloat(),
               (int)bbl.at(2).toFloat(), (int)bbl.at(3).toFloat());
  ...
  int mtype_final = -1;
  int guess_count = 0;
  for (node = agfstnode(G); node && /*mtype_final==-1*/ guess_count <= 10;
  ...
  */
  ...
  for (node = agfstnode(G); node; node = agnxtnode(G, node)) {
    ...
    pen.setWidth(1);
    m->addState(nname, "", 0, nmoore, xpos + graph_offsetx,
                ypos + graph_offsety, (int)(nwidth / 2.0), 1.0, pen, endstate);
  }
  ...
  for (node = agfstnode(G); node; node = agnxtnode(G, node)) {
    ...
    if (cur_state) {
      for (edge = agfstout(G, node); edge; edge = agnxtout(G, edge)) {
        ...
        // qDebug("%s -> %s", nname.toLatin1().constData(),
        // enname.toLatin1().constData());
        ...
        int eind = -1;
        int cpnum =
            0; // cpnum holds the number of control points stored in stx, sty
        for (int i = 0; i < sllen; i++) {
          ...
          {
            // If we have 5 control points we can map the spline directly to a
            // cubic spline. Otherwise we store all control points in stx, sty
            // qDebug("control point %d", i);
            stx[cpnum] =
                sl.at(i).section(",", -2, -2).toFloat() + graph_offsetx;
            sty[cpnum] = bb.height() - sl.at(i).section(",", -1, -1).toFloat() +
                         graph_offsety;
            // qDebug("%f %f   %f %f", sl.at(i).section(",",-1,-1).toFloat(),
            // sl.at(i).section(",",-2,-2).toFloat(), tx[cpnum], ty[cpnum]);
            ind++;
            cpnum++;
          } else
            ind++;
        ...
        if (sllen == 5) {
          // For 5 control points: direct mapping to cubic spline (while leaving
          // out second last point)
          ind = 0;
          for (int i = 0; i < 5; i++) {
            if (i == eind)
              continue;
            cpx[ind] = sl.at(i).section(",", -2, -2).toFloat() + graph_offsetx;
            cpy[ind] = bb.height() - sl.at(i).section(",", -1, -1).toFloat() +
                       graph_offsety;
            ind++;
          }
          if (eind >= 0) {
          ...
          if (cpx[3] - cpx[2] != 0)
            ...
            angle2 = M_PI_2;
          if (fabs(angle1 - mainangle) < straight_thresh &&
              fabs(angle2 - mainangle) < straight_thresh) {
            straight = true;
            // qDebug("straight");
          ...
          // and we have to approximate the spline segments with a single cubic
          // spline
          tx[0] = stx[0];
          ty[0] = sty[0];
          tx[maxpoints - 1] = tmpx;
          ...
          if (fabs(angle1 - mainangle) < straight_thresh &&
            ...
            if (cur_state == end_state) {
              ...
              cur_state->calcLoop(sxpos, sypos, cur_state->getRadius(), cpx[0],
                                  cpy[0], cpx[1], cpy[1], cpx[2], cpy[2]);
              straight = true;
            } else {
              straight = false;
              ...
              float *slength =
                  new float[nsplines]; // approximate length of splines
              float *cumlen =
                  new float[nsplines]; // accumulative length of splines
              float sumlen = 0;
              ...
              for (int di = 0; di < maxpoints - 1; di++) {
                if ((float)di / maxpoints > cumlen[curspline] &&
                    curspline < nsplines - 1) {
                ...
                t = float(di - lastind) /
                    (maxpoints * (slength[curspline] / sumlen));
                if (t > 1)
                  t = 1;

                oneminust = 1.0 - t;
                tx[di] =
                    oneminust * oneminust * oneminust * stx[curspline * 3] +
                    3 * oneminust * oneminust * t * stx[curspline * 3 + 1] +
                    3 * oneminust * t * t * stx[curspline * 3 + 2] +
                    t * t * t * stx[curspline * 3 + 3];
                ty[di] =
                    oneminust * oneminust * oneminust * sty[curspline * 3] +
                    3 * oneminust * oneminust * t * sty[curspline * 3 + 1] +
                    3 * oneminust * t * t * sty[curspline * 3 + 2] +
                    t * t * t * sty[curspline * 3 + 3];
                /*
                qDebug("%f  %f", tx[di], ty[di]);
                if (di>0)
                {
                  qDebug("           dist %f",
                sqrt((tx[di]-tx[di-1])*(tx[di]-tx[di-1])+(ty[di]-ty[di-1])*(ty[di]-ty[di-1])));
                }
                */
              }
        ...
        cur_state->addTransition(project, end_state, ti, cpx[0], cpy[0], cpx[3],
                                 cpy[3], cpx[1], cpy[1], cpx[2], cpy[2], "",
                                 straight, false);
      }
    }
    scount++;
...
void ImportGraphviz::cubicSplineApprox(int numin, double x[], double y[],
  ...
  if (cpx[3] < cpx[0])
    mainangle += M_PI;
  double maindist = sqrt((cpx[0] - cpx[3]) * (cpx[0] - cpx[3]) +
                         (cpy[0] - cpy[3]) * (cpy[0] - cpy[3]));
  ;
  ...
  */
  ...
  while (!convergence) {
    ...
    distc1 = sqrt((newcx1 - cpx[0]) * (newcx1 - cpx[0]) +
                  (newcy1 - cpy[0]) * (newcy1 - cpy[0]));
    distc2 = sqrt((newcx2 - cpx[3]) * (newcx2 - cpx[3]) +
                  (newcy2 - cpy[3]) * (newcy2 - cpy[3]));
    if (cpx[3] - newcx2 != 0)
    ...
    if (distc1 > 0.2 * maindist && distc1 < 0.8 * maindist &&
        fabs(angle1 - mainangle) < M_PI) {
      if (newcx1 > 0)
        cpx[1] = newcx1;
      if (newcy1 > 0)
        cpy[1] = newcy1;
    }
    if (distc2 > 0.2 * maindist && distc2 < 0.8 * maindist &&
        fabs(angle2 - mainangle) < M_PI_2) {
      if (newcx2 > 0)
        cpx[2] = newcx2;
      if (newcy2 > 0)
        cpy[2] = newcy2;
