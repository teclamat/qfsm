...
QString TableBuilderHTML::fileFilter() { return "HTML State Table (*.html)"; }

QString TableBuilderHTML::defaultExtension() { return "html"; }
...
QString TableBuilderHTML::getHead() {
  ...
  sheadhead += "\t<TH valign=\"bottom\" rowspan=\"2\">" + row_label + "</TH>\n";

  sheadhead += "\t<TH colspan=\"" + QString::number(rowcount) +
               "\" halign=\"center\">" + col_label + "</TH>\n";
  sheadhead += "</TR>\n";
  sheadhead += "<TR>\n";
  ...
  if (options->getStateTableOrientation() == 0) {
    for (; ioit.hasNext();) {
      ...
      if (info->isInverted())
        ...
        nottmp = "";
      shead += "\t<TH>" + nottmp + " " +
               info->convertToString(machine, options) + "</TH>\n";
    }
  } else {
    for (; sit.hasNext();) {
      s = sit.next();
...
QString TableBuilderHTML::getRow(IOInfo *io) {
  ...
  if (io->isInverted())
    ...
    nottmp = "";
  srow += "\t<TD>" + nottmp + " " + io->convertToString(machine, options) +
          "</TD>\n";
