...
ExportTestvectorASCII::ExportTestvectorASCII(Options *opt) : Export(opt) {
...
ExportTestvectorASCII::~ExportTestvectorASCII() { delete generator; }
...
void ExportTestvectorASCII::doExport() {
  ...
  generator->init(machine, synchronous_reset, synchronous_enable,
                  negated_reset);
...
QString ExportTestvectorASCII::fileFilter() { return "Testvector (*.vec)"; }

QString ExportTestvectorASCII::defaultExtension() { return "vec"; }
    ...
    *out << "# --------------" << endl;
    foreach (const QString &input, machine->getInputNameList())
      *out << "# " << input.toLatin1().constData() << " : "
           << bit_string.toLatin1().constData() << ";" << endl;
    *out << endl;
  }
