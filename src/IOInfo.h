...
public:
  ...
  virtual QString convertToString(Machine *m = NULL,
                                  Options *opt = NULL) const = 0;
  /// Returns a comparable string representing the input/output information
  virtual QString convertToComparableString(Machine *m = NULL,
                                            Options *opt = NULL) const = 0;
  /// Returns a string representing the binary input/output information
  virtual QString convertToBinStr(Machine *m = NULL,
                                  Options *opt = NULL) const = 0;
   ...
   */
  virtual void setASCII(const unsigned char *ascii, int asciilen,
                        int numbits) = 0;
  ...
  virtual QList<IOInfo *> getSingles() = 0;
  /// Returns a list of IOInfo objects with single characters representing this
  /// I/O info (does take into account inversion)
  ...
  virtual bool isRange(int &start, int &end) const {
    start = range_start;
    end = range_end;
    return range;
  };
  ...
  /// Returns 0 is a binary ioinfo, 1 for ASCII, 2 for free text and -1 if
  /// ambiguous
