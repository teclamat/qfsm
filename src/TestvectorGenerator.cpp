...
void TestvectorGenerator::generateTestvector() {
  ...
  if (synchronous_reset || synchronous_enable)
    ...
    progress.setRange(0, num_transitions);
  ...
  while (true) {
    ...
    while (is.hasNext()) {
      ...
      while (it.hasNext()) {
        ...
        if (t->getEnd() == s1) {
          ...
          message = "Transition: " + s2->getStateName() + " -> " +
    ...
    while (it.hasNext()) {
      ...
      if (!t->checkMark()) {
        ...
        if (s2 == NULL)
          continue;
        message = "Transition: " + s1->getStateName() + " -> " +
                  t->getInfo()->getInputsStr(machine) + " -> " +
                  s2->getStateName();
    ...
    if (!ctl_in_finished) {
      if (synchronous_enable) {
        for (int c = 0; c < (1 << (machine->getNumInputs())); c++) {
          ...
          if (synchronous_enable)
            controlInput.append("0");
          input_vector.append(
              new IOInfoBin(IO_MealyIn, c, machine->getNumInputs(), false));
          moore_output_vector.append(s2->getMooreOutputs());
          mealy_output_vector.append(t->getInfo()->getOutputInfo());
          control_signal_vector.append(
              new IOInfoBin(IO_MealyIn, controlInput, false));
      ...
      if (synchronous_reset) {
        ...
        s1 = machine->getInitialState();
        input_vector.append(
            new IOInfoBin(IO_MealyIn, 0, machine->getNumInputs(), false));
        moore_output_vector.append(s1->getMooreOutputs());
        mealy_output_vector.append(t->getInfo()->getOutputInfo());
        control_signal_vector.append(
            new IOInfoBin(IO_MealyIn, controlInput, false));
    ...
    /*      else {
            ...
            if(synchronous_enable)
              controlInput.append("1");
            control_signal_vector.append(new
       IOInfoBin(IO_MealyIn,controlInput,false));

          }*/
  }
  machine->resetMarks();
  message =
      "Size Moore Outputs: " + QString::number(moore_output_vector.size());
  message +=
      " Size Mealy Outputs: " + QString::number(mealy_output_vector.size());
  ...
  while (it.hasNext()) {
    ...
    if (!found) {
      ...
      if (t->getEnd() != NULL) {
        ...
        if ((return_list1.size() < return_list2.size() &&
             !return_list1.isEmpty()) ||
